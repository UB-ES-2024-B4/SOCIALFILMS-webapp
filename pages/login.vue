  ...
  try {
    ...
    switch (error.code) {
      ...
      case authErrorCodes.EmailNotConfirmed:
        toast.add({severity: "warn", summary: 'Email no confirmat', detail:'Si us plau, confirma el teu email per accedir al teu compte.', life: 6500})
        break
    }
      ...
      <form class="flex flex-col gap-y-3" @submit.prevent="handleLogin">
        ...
        <div class="flex flex-col">
          <InputGroup>
            ...
            <FloatLabel variant="on">
              <Password id="on_label_password" v-model="credentials.password" :invalid="credentialsStatus.failedAuth" :feedback="false" toggleMask />
              <label for="on_label_password">Contrasenya</label>
            </FloatLabel>
          </InputGroup>

          <transition name="p-message" mode="out-in">
            <Message class="mt-1.5" v-if="credentialsStatus.invalidCredentials" severity="error" icon="pi pi-times-circle">Credencials invàlides</Message>
          </transition>
        </div>
