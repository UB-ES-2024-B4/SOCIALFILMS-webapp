...
CREATE OR REPLACE FUNCTION public.get_now_playing_movies(lang text)
 RETURNS json
 LANGUAGE plpgsql
AS $function$declare
  request_url text;
  ...
  result json;
BEGIN
  request_url := format('https://api.themoviedb.org/3/movie/now_playing?language=%s&page=1&api_key=%s', lang, api_key);
