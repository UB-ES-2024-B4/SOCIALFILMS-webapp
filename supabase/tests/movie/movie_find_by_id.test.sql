...
SELECT ok(
  (SELECT public.find_movie_by_id(98,null) IS NOT NULL), 
...
SELECT ok(
  (SELECT (result->>'title') IS NOT NULL 
   FROM (SELECT public.find_movie_by_id(98,null) AS result) AS subquery),
...
SELECT ok(
  (SELECT (result->>'title')
   FROM (SELECT public.find_movie_by_id(98,null) AS result) AS subquery) = 'Gladiator',
